    ...
    <Box sx={{ width: "30%", display: "flex", alignItems: "stretch" }}>
      <Card variant="outlined" sx={{ display: "flex", flexDirection: "column", flexGrow: 1 }}>
        <CardContent sx={{ display: "flex", flexDirection: "column", flexGrow: 1 }}>
          <Box
            component="img"
            sx={{ display: "block", width: "220px", height: '220px', mx: "auto" }}
            src={selectImage()}
            alt="Фото Запроса"
          />
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Typography variant="subtitle2" sx={{ mt: 2 }}>Локация</Typography>
            <Typography variant="body2">{location.district}, {location.city}</Typography>

            <Typography variant="subtitle2" sx={{ mt: 2 }}>Цель сбора</Typography>
            <Typography variant="body2">{goalDescription}</Typography>

            <Typography variant="subtitle2" sx={{ mt: 2 }}>Завершение</Typography>
            <Typography variant="body2">{new Date(endingDate).toLocaleDateString()}</Typography>
          </Box>

          <Box>
            <Typography variant="subtitle2" sx={{ mt: 2 }}>Мы собрали</Typography>
            <LinearProgress variant="determinate" value={progress} color="secondary" />
            <Box sx={{ display: "flex", justifyContent: "space-between", mt: 1, mb: 2 }}>
              <Typography variant="body2">{collectedAmount.toLocaleString()} руб</Typography>
              <Typography variant="body2">из {targetAmount.toLocaleString()} руб</Typography>
            </Box>

            <Typography variant="body2" color="textSecondary" mt={2}>
              Нас уже {contributorsCount.toLocaleString()}
            </Typography>
            <Button variant="contained" color="primary" sx={{ mt: 2, width: "100%" }} onClick={onClick}>
              Помочь
            </Button>
          </Box>
        </CardContent>
